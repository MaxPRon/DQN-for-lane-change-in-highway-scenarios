...
class World:
    ...
    def reward_function(self):
        ...
        for lane in range(1, self.n_lanes + 1):
            if self.vehicle_list[0].y > (lane - 1) * self.road_width + self.road_width * 0.5 - epsilon and self.vehicle_list[0].y < (lane - 1) * self.road_width + self.road_width * 0.5 + epsilon:
                self.reward+=2
        #### Veloccity Delta ####
        #non_ego_avg_v = 0
        #for n in range(0,self.n_cars):
