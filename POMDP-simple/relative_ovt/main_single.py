...
input_dim_v = (x_view*2+1)*(y_view*2+1)*2
...
for r_seed in range(1,random_sweep):
    start = time.time()
    ...
    path_save = folder_path+ "testing_02/"
    ...
    with tf.Session() as sess:
        ...
        for episode in range(max_train_episodes):
            ...
            if episode % average_window == 0:
                if episode > 1:
                    print("Total steps: ", total_steps," Episode: ",episode, " Average reward over "+str(average_window)+" Episodes: ",
                          np.mean(reward_sum_list[r_seed,episode-average_window:episode]),"Finished: ",finished,"/"+str(average_window)+" Episode:", episode)
                    ...
                    file.write("Total steps: "+str(total_steps)+" Episode: "+ str(episode)+" Average reward over "+str(average_window)+" Episodes: "+
                          str(np.mean(reward_sum_list[r_seed,episode-average_window:episode]))+"Finished: "+str(finished)+"/"+str(average_window)+"\n")
            ...
            if r_seed == 0 and episode == 1:  # Only write for first time
                ...
                file.write('REMARKS: First Test with Partial Observavility, longer training')
