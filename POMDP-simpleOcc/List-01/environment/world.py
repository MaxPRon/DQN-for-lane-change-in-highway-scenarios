...
class World:
    ...
    def step(self,action):
        ...
        #### Observation ####
        self.observation = self.limited_view()
        self.observation = self.relative_state(self.observation)
    ...
    def limited_view(self):
        ...
        self.observation = copy.deepcopy(self.vehicle_list_c)
        epsilon = 2
        max_x_prev = 0
        for car_no in range(1, self.n_cars + 1):
            max_x = self.observation[car_no].x
            ...
            if abs(dist_x) > self.x_view or abs(dist_y) > self.y_view:
                self.observation[car_no].x = 0
                self.observation[car_no].y = 0
                self.observation[car_no].v = 0
            if max_x > max_x_prev:
                index_max = car_no
                max_x_prev = max_x

        if self.observation[index_max].y > self.observation[0].y - epsilon and self.observation[index_max].y < self.observation[0].y + epsilon:
            self.observation[index_max].x = 0
            self.observation[index_max].y = 0
            self.observation[index_max].v = 0
